				...
				<v-col cols="6" md="6" sm="10" class="v-col-xs-12">
					<v-card class="pa-3" :color="record.type === 'outcome' ? 'red-lighten-1' : 'light-green-lighten-1'">
						<v-card-text class="text-subtitle-1 text-primary">
							...
							<p class="mt-3">{{ t('amount') + ': ' + n(cf(record.amount), 'currency', userCurrency) }}</p>
							...
							<div v-if="record.details?.length" class="record__details mt-4">
								...
								<p v-for="detail in record.details" :key="detail.id" class="record__detail mb-2 text-fixed">
							...
							<small class="text-right d-block mr-1">{{ d(record.created_at, 'short') }}</small>
						</v-card-text>
					</v-card>
				</v-col>
			</v-row>
