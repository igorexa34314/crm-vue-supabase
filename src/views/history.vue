	...
	<div>
		...
		<section v-if="recordsState?.records && recordsState.records.length" class="mt-lg-6">
			<RecordsTable
				...
				v-bind="{ sortField: sortState.field, sortType: sortState.type }"
				@sort="sortRecords" />
		...
		<div
			v-else-if="!catLoading && !recLoading && (!recordsState?.records || !recordsState?.records.length)"
...
const sortState = computed({
	get: () => ({
		field: ['id', 'created_at', 'amount', 'category_id', 'description', 'type'].includes(route.query.sort as string)
			? (route.query.sort?.toString() as SortFields)
			: 'created_at',
...
watch(page, async newPage => {
	try {
		...
		const records = await RecordService.loadMoreRecords({
			sortBy: sortState.value.field,
			sortType: sortState.value.type,
			page: newPage,
			perPage,
